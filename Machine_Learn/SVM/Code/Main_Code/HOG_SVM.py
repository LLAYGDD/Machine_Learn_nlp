...
from ulits.__init__ import image_width
from ulits.__init__ import image_height
...
train_image_path =dir_path+datasets_2
test_image_path = dir_path+datasets_2

train_label_path = os.path.join(train_image_path,'train.txt')
test_label_path = os.path.join(test_image_path,'train.txt')
train_feat_path = dir_path+'train/'
test_feat_path = dir_path+'test/'
...
def get_image_list(filepath,namelist):
    ...
    for  name in namelist:
        ...
        image_list.append(temp.copy())
        temp.close()
    return image_list

def get_feature(image_list,name_list,label_list,savepath):
    print("extract feature of image!")
    i=0
    for image in image_list:
        try:
            image=np.reshape(image,(image_height,image_width,3))
        except:
            print("abnormal of size for image!",name_list[i])
            continue
        gray = rgb2gray(image) / 255.0
        # 这句话根据你的尺寸改改
        fd = hog(gray, orientations=12,
                 block_norm='L1',
                 pixels_per_cell=[8, 8],
                 cells_per_block=[4, 4],
                 visualise=False,
...
def get_name_label(file_path):
    print("read labe from image!",file_path)
    name_list=[]
    label_list=[]
        ...
        for line in f.readlines():
            if len(line)>=3:
                ...
                if not str(label_list[-1]).isdigit():
                    print("label 必须是一个数字",label_list[-1])
                    exit(1)
